...
def monitor_cluster(config_sources):
    ...
    config = Config(region_name=aws_region, connect_timeout=boto_timeout, retries={'max_attempts': boto_retries})
    ...
    user = get_config_value(['DbUser', 'db_user', 'dbUser', 'DatabaseUser'], config_sources)
    host = get_config_value(['HostName', 'cluster_endpoint', 'dbHost', 'db_host', 'ClusterEndpoint'], config_sources)
    port = int(get_config_value(['HostPort', 'db_port', 'dbPort', 'ClusterPort'], config_sources))
    ...
    global interval
    interval = get_config_value(['AggregationInterval', 'agg_interval', 'aggregtionInterval', 'ScheduleFrequency'], config_sources)
    ...
    set_name = "set application_name to 'AuroraPostgresAdvancedMonitoring-v%s'" % __version__
    ...
    put_metrics = run_external_commands('Aurora Postgres Diagnostic', 'monitoring-queries.json', cursor, cluster)
